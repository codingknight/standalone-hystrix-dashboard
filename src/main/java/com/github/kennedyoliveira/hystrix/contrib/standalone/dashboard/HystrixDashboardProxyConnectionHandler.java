...
public class HystrixDashboardProxyConnectionHandler implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext requestCtx) {
        String proxyUrl = getProxyUrl(requestCtx);
        if (proxyUrl != null) {
            URI uri = createUriFromUrl(proxyUrl, requestCtx);
            if (uri != null) {
    ...
    void proxyRequest(URI proxiedUrl, RoutingContext requestCtx) {
        ...
        httpClientRequest.handler(new Handler<HttpClientResponse>() {
            ...
            public void handle(HttpClientResponse clientResponse) {
                ...
                if (clientResponse.statusCode() == 200) {
                    ...
                    MultiMap map = clientResponse.headers();
                    for (Map.Entry<String, String> headerEntry : map.entries()) {
        ...
        requestCtx.response().closeHandler(new Handler<Void>() {
            @Override
        ...
        serverRequest.exceptionHandler(new Handler<Throwable>() {
            @Override
            public void handle(Throwable t) {
