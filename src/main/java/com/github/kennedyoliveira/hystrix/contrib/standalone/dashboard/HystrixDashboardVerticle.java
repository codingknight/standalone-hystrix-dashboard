...
public class HystrixDashboardVerticle extends AbstractVerticle {
    ...
    public void start(final Future<Void> startFuture) throws Exception {
        vertx.sharedData().getCounter("instance-count", new Handler<AsyncResult<Counter>>() {
            @Override
            public void handle(AsyncResult<Counter> counterAsync) {
                if (counterAsync.failed()) {
                    startFuture.fail(counterAsync.cause());
                } else {
                    counterAsync.result()
                            .incrementAndGet(new Handler<AsyncResult<Long>>() {
                                @Override
                                public void handle(AsyncResult<Long> event) {
                                    if (event.failed()) {
                                        startFuture.fail(event.cause());
                                    } else {
                                        log.info("Initializing the HystrixDashboardVerticle instance {}", event.result());
     ...
     */
    private void startServer(final Future<Void> startFuture, final Router mainRouter, final HttpServerOptions options) {
        vertx.createHttpServer(options)
                .requestHandler(new Handler<HttpServerRequest>() {
                    @Override
                    public void handle(HttpServerRequest event) {
                        mainRouter.accept(event);
                    }
                })
